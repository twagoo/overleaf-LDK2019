...
At this time, the EDM full-text profile is already being applied at production-level. Europeana has converted the Europeana Newspapers corpus to the EDM full-text profile. , Therefore, the profile has been applied to more than 10 million pages of real newspaper full-text data, in multiple languages and scripts. Since this corpus originates from several data providers from different countries using different practices for digitisation, we see this application also as evidence that the model can accommodate the different ways of structuring full-text in digitised objects. 
